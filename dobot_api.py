...
alarmControllerFile = "files/alarmController.json"  # 控制器告警信息JSON文件路径
alarmServoFile = "files/alarmServo.json"    # 伺服电机告警信息JSON文件路径
...
def alarmAlarmJsonFile():
    ...
    jsonContrellorPath = os.path.join(currrntDirectory, alarmControllerFile)    # 拼接控制器告警文件完整路径
    jsonServoPath = os.path.join(currrntDirectory, alarmServoFile)  # 拼接伺服告警文件完整路径

    with open(jsonContrellorPath, encoding='utf-8') as f:   # 读取控制器告警JSON
        dataController = json.load(f)
    with open(jsonServoPath, encoding='utf-8') as f:    # 读取伺服告警JSON
...
class DobotApi:
    def __init__(self, ip, port, *args):
        self.ip = ip
        self.port = port
        self.socket_dobot = 0   # 初始化socket对象（0表示未连接）
        self.__globalLock = threading.Lock()    # 线程锁（保证发送/接收同步）
        if args:
            self.text_log = args[0]

        # 仅支持指定端口（29999、30004、30005，机械臂常用通信端口）
        if self.port == 29999 or self.port == 30004 or self.port == 30005:
            try:
                self.socket_dobot = socket.socket() # 创建TCP socket
                self.socket_dobot.connect((self.ip, self.port)) # 连接机械臂
                self.socket_dobot.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 144000)   # 设置接收缓冲区大小
            except socket.error:
                print(socket.error)
    ...
    def wait_reply(self):
        ...
        data = ""
        try:
            data = self.socket_dobot.recv(1024) # 接收最多1024字节数据
        except Exception as e:
            print(e)
            self.socket_dobot = self.reConnect(self.ip, self.port)
    ...
    def close(self):
        ...
        if (self.socket_dobot != 0):
            try:
                self.socket_dobot.shutdown(socket.SHUT_RDWR)    # 关闭读写
                self.socket_dobot.close()   # 关闭socket
            except socket.error as e:
                print(f"Error while closing socket: {e}")

    #封装 “发送指令 - 接收反馈” 的同步逻辑，通过线程锁确保通信过程的原子性
    ...
    #析构函数，确保实例对象被销毁时关闭 TCP 连接，释放资源
    def __del__(self):
        self.close()

    #网络断开时的重连逻辑，循环尝试连接直到成功，保证通信的连续性
...
class DobotApiDashboard(DobotApi):
    ...
    def EnableRobot(self, load=0.0, centerX=0.0, centerY=0.0, centerZ=0.0, isCheck=-1,):
        """
            ...
            isCheck     int       是否检查负载。1表⽰检查，0表⽰不检查。如果设置为1，则机械臂使能后会检查实际负载是否和设置负载⼀致，如果不⼀致会⾃动下使能。默认值为0
        ...
        string = 'EnableRobot('
        if load != 0:   # 若指定负载
            string = string + "{:f}".format(load)   # 指令里添加负载参数
            if centerX != 0 or centerY != 0 or centerZ != 0:    # 若指定偏心距
                string = string + ",{:f},{:f},{:f}".format(
                    centerX, centerY, centerZ)  # 指令里添加偏心距
                if isCheck != -1:   # 若指定是否检查
                    string = string + ",{:d}".format(isCheck)   # 指令里添加检查标志
        string = string + ')'
        return self.sendRecvMsg(string) # 发送使能指令并返回反馈
    ...
    def DisableRobot(self):
        ...
        """
        string = "DisableRobot()"
        return self.sendRecvMsg(string) # 发送下使能指令
    ...
    def ClearError(self):
        ...
        """
        string = "ClearError()"
        return self.sendRecvMsg(string) # 发送清除错误的指令
    ...
    def PowerOn(self):
        ...
        """
        string = "PowerOn()"
        return self.sendRecvMsg(string) #控制机械臂上电，为使能做准备（上电后约 10 秒才能使能）
    ...
    def RunScript(self, project_name):
        ...
        return self.sendRecvMsg(string) #运行机械臂预存的脚本文件
    ...
    #：控制指定关节的抱闸（仅在机械臂下使能时有效），value=1松开抱闸可手动拖拽关节，value=0锁死固定关节。
    ...
    def SetTool485(self, baud, parity='', stopbit=-1, identify=-1):
        """
        描述:
    ...
    def MovJ(self, a1, b1, c1, d1, e1, f1, coordinateMode, user=-1, tool=-1, a=-1, v=-1, cp=-1):
        """
        描述
        从当前位置以关节运动⽅式运动⾄⽬标点(x,y,z,Rx,Ry,Rz)或者（J1，J2，J3，J4，J5，J6）
        必选参数
        参数名      类型      说明
